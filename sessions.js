    ...
    static async closeSession(sessionName) {
        ...
        if (session) { //só adiciona se não existir
            if (session.state != "CLOSED") {
                ...
                return { result: "success", state: session.state, status:  session.status};
            } else { //close
                return { result: "success", state: session.state, status:  session.status};
            }
        } else {
        ...
        if (session) {
            if (session.state == "CONNECTED") {
               ...
               if(numVerifica === 200){
                    ...
                    return { result: "success", state: session.state };
                } else {
                    return { result: "error", state: session.state };
                }
            } else {
                return { result: "error", state: session.state };
            }
        } else {
            return { result: "error", state: "NOTFOUND" };
        }
    } //message
        ...
        if (session) {
            if (session.state == "CONNECTED") {
                var resultSendFile = await session.client.then(async (client) => {
                    return await client.sendFile(number + '@c.us', pathFile, fileName, caption);
                }); //client.then(
