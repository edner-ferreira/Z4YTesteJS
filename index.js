...
app.get("/start", async (req, res, next) => {
    ...
    // = process.env.JSONBINIO_SECRET_KEY ?
    ...
    if (["CONNECTED", "QRCODE", "STARTING"].includes(session.state)) {
        res.status(200).json({ result: 'success', state: session.state, status : session.status });
    } else {
        res.status(200).json({ result: 'error', state: session.state });
    }
});//start

app.get("/getSession", async (req, res, next) => {
    
    var session = Sessions.getSession(req.query.sessionName);
    //console.log(session);

    if(session === false) {
        // console.log("result : error , state : " + session.state);
        res.status(200).json({result : "error", state : session.state});
    }
    else{
        // console.log("result : success , state : " + session.state);
        res.status(200).json({result: "success", state: session.state, status: session.status});
    }  
});
...
app.post("/sendText", async function sendText(req, res, next) {
    ...
    if(result.result === "success"){
        console.log('MSG enviada para ' + req.query.number + ' - OK');
    } else {
        console.log('MSG enviada para ' + req.query.number + ' - FALHOU PQ NAO TEM ZAP!!!');
    }
    res.status(200).json({result: result.result, state: result.state});
});//sendText
